...
pockets._ = {};
pockets.ROOT = pockets._;
/**
 *
 * @param options object
 * - mock [boolean] setting to `true` will skip balance retrieval.
...
pockets.list = function (options, callback) {
  ...
  async.series(calls, function (err) {
    if (err)
      return callback(err);
    engine.bitcoin.balance(pockets._, function (err, balance) {
      if (err)
        return callback(err);
      if (pockets._ && pockets._.wallet)
...
pockets.get = function (options, callback) {
  ...
  if (pockets._.name === options.name)
    return callback(null, pockets._);
  var found = recurse(pockets._, options.name);
  if (found) {
    if (options.mock)
...
pockets.delete = function (options, callback) {
  ...
  if (pockets._[options.name] === options.name)
    return callback(null, pockets._ = {});
...
pockets.init = function (options, callback) {
  var value = engine.db.get('pockets.json');
  if (value)
    pockets._ = value;
  else
    pockets._ = {};
  ...
  return pockets.realign({mock: true}, function (err) {
    return pockets.ensureLevelPercentages({}, function (err) {
      return pockets.realign({}, function (err) {
        if (err)
          return callback(err);
        engine.db.set('pockets.json', pockets._);
...
pockets.realign = function (options, callback) {
  ...
  console.log(engine.pockets._);
  if (!engine.pockets._ || (pockets._ && !pockets._.name)) {
  ...
  pockets.totalBalance({}, function (err, balance) {
    ...
    return engine.pockets.get({name: 'root'}, function (err, rootPocket) {
      ...
      var childBalance = 0;
      traverse.map(pockets._, function (y) {
        if (y && typeof y === 'object' && y.name && y.leaf && y.wallet) {
          childBalance += y.wallet.balance || 0;
        }
      });
      pockets._.child_balance = childBalance;
      ...
      traverse.forEach(rootPocket, function (x) {
        ...
        if (x && typeof x === 'object' && point.parent && x.name) {
          ...
          if (parent.pockets) {
            ...
            if (!x.leaf) {
              var childBalance = 0;
              traverse.map(x, function (y) {
                if (y && typeof y === 'object' && y.name && y.leaf && y.wallet) {
                  childBalance += y.wallet.balance || 0;
                }
              });
              x.child_balance = childBalance;
            }
